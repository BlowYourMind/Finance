...
export class OkexService {
  ...
  async sign(path: string, postData: any, nonce: string, method: string = 'POST') {
  ...
  async waitForValue(path: string, postData: any, signature: string, nonce: string, method: string = 'POST') {
    const command = method === 'POST' ? 'post' : 'get';
    this.httpService.get('', { headers: SignatureService.createOKXHeader(signature, nonce) })
  ...
  async getInstrument(asset: string): Promise<OkexInstrument> {
    ...
    const path = OkexUrls.INSTRUMENTS + '?' + new URLSearchParams({ instType: 'FUTURES', uly: OkexUlys[asset.toUpperCase()] }).toString();
    const signature = await this.sign(path, {}, nonce, 'GET')
    const instruments = await this.waitForValue(path, {}, signature, nonce, 'GET');
  ...
  async getOrder(orderId: string, instId: string) {
    const nonce = new Date().toISOString();
    const path = OkexUrls.ORDER + new URLSearchParams({ ordId: orderId, instId: instId }).toString();
    const signature = await this.sign(path, {}, nonce, 'GET');
  ...
  async futureBuy(amount: string, asset: string, approxStableValue: string): Promise<string> {
    log(`[INFO] Shorting Future on amount of ${amount} ${asset} with ${approxStableValue} USDT`.cyan);
    ...
    const res = (await this.waitForValue(OkexUrls.ORDER, postData, signature, nonce)).data;
  ...
  async setLeverage(asset: string) {
    ...
    const res = await this.waitForValue(OkexUrls.SET_LEVERAGE, postData, signature, nonce);
  }
  ...
  async futureSell(amount: string, asset: string) {
    ...
    const signature = await this.sign(OkexUrls.ORDER, postData, nonce);
    const res = await this.waitForValue(OkexUrls.ORDER, postData, signature, nonce);
    const balance = await this.check('usdt');
  }

  async moveFundsFromTo(amount: string, asset: string, from: OkexWallets, to: OkexWallets): Promise<boolean> {
    const nonce = new Date().toISOString();
    const postData = {
      ccy: asset,
      amt: amount,
    ...
    const balance = await this.waitForValue(OkexUrls.TRANSFER, postData, signature, nonce);
    return balance ? true : false;
  }

  async getCurrency(asset: string) {
    const nonce = new Date().toISOString();
    const path = OkexUrls.CURRENCIES + '?' + new URLSearchParams({ ccy: asset }).toString();
    const signature = await this.sign(path, undefined, nonce, 'GET');
    const balance = await this.waitForValue(path, undefined, signature, nonce, 'GET');
    return balance.data;
  }

  async transfer(asset: string, amount: string, address: string) {
    const nonce = new Date().toISOString();
    const fee = (await this.getCurrency(asset)).data.filter(elem => elem.chain === 'ETH-ERC20')[0];
    ...
    const balance = await this.waitForValue(OkexUrls.WITHDRAW, postData, signature, nonce);
    return balance.data;
  }
  ...
  async check(asset?: string): Promise<BalanceInfo> {
    const nonce = new Date().toISOString();
    const params = asset ? '?' + new URLSearchParams({ ccy: asset }).toString() : '';
